...
impl DwarfData {
    ...
    pub fn get_function_data_from_addr(&self, curr_addr: usize) -> Option<&Function> {
        self.files.iter().find_map(|file| {
            file.functions.iter().find(|func| {
                let start = func.address;
                let end = func.address.saturating_add(func.text_length.max(1));
                curr_addr >= start && curr_addr < end
            })
        })
    }

    pub fn get_local_variable_from_addr(&self, curr_addr: usize, name: &str) -> Option<&Variable> {
        self.get_function_data_from_addr(curr_addr)?
            .variables
            .iter()
    ...
    pub fn resolve_variable_location<'a>(
        &self,
        curr_addr: usize,
        variable: &'a Variable,
    ) -> &'a Location {
        if let Some(scoped) = variable
            .location_list
            .iter()
            .find(|entry| curr_addr >= entry.range_start && curr_addr < entry.range_end)
        {
            &scoped.location
        } else {
            &variable.location
...
impl fmt::Display for Location {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            ...
            Location::Register(register) => write!(f, "Register({})", register),
        }
    }
}
...
pub struct Variable {
    ...
    pub location: Location,
    pub location_list: Vec<ScopedLocation>,
