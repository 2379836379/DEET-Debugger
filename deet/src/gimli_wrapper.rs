...
use crate::dwarf_data::{File, Function, Line, Location, ScopedLocation, Type, Variable};
...
pub fn load_file(object: &object::File, endian: gimli::RunTimeEndian) -> Result<Vec<File>, Error> {
    ...
    let mut load_section = |id: gimli::SectionId| -> Result<borrow::Cow<[u8]>, Error> {
        match object.section_by_name(id.name()) {
            Some(section) => section
                .uncompressed_data()
                .map_err(|e| Error::ObjectError(e.to_string())),
            None => Ok(borrow::Cow::Borrowed(&[][..])),
        }
    };

    // Load all of the sections.
    let dwarf_sections = gimli::DwarfSections::load(&mut load_section)?;
        ...
        while let Some(entry) = entries.next_dfs()? {
            ...
            match entry.tag() {
                ...
                gimli::DW_TAG_base_type => {
                    ...
                    let byte_size = if let Some(attr) = entry.attr(gimli::DW_AT_byte_size) {
                ...
                gimli::DW_TAG_formal_parameter | gimli::DW_TAG_variable => {
                    ...
                    let mut location_list: Vec<ScopedLocation> = Vec::new();
                    ...
                    for attr in entry.attrs() {
                        ...
                        match attr.name() {
                            ...
                            gimli::DW_AT_location => {
                                location_list = get_location_list(&attr, &unit, &dwarf);
                                if let Some(loc) = get_location(&attr, &unit, &dwarf) {
                                    location = Some(loc);
                                }
                            }
                            gimli::DW_AT_decl_line => {
                    ...
                    if entity_type.is_some() && (location.is_some() || !location_list.is_empty()) {
                        let is_parameter = entry.tag() == gimli::DW_TAG_formal_parameter;
                        let mut var = Variable {
                            name,
                            entity_type: entity_type.unwrap(),
                            location: location.or_else(|| location_list.first().map(|entry| entry.location.clone())).unwrap(),
                            location_list,
                        ...
                        if depth == 1 {
                            ...
                            let func = compilation_units
                                ...
                                .unwrap();
                            if is_parameter {
                                var.parameter_index = Some(func.parameter_count);
...
fn parse_expression_location<R: Reader>(
    ...
    if let Ok(op) = gimli::Operation::parse(&mut pc, encoding) {
        match op {
            gimli::Operation::FrameOffset { offset } => {
                Some(Location::FramePointerOffset(offset.try_into().unwrap()))
            }
            gimli::Operation::Address { address } => {
                Some(Location::Address(address.try_into().unwrap()))
            }
            gimli::Operation::Register { register } => Some(Location::Register(register.0)),
            _ => None,
        }
    } else {
        None
...
fn get_location<R: Reader>(
    attr: &gimli::Attribute<R>,
    unit: &gimli::Unit<R>,
    dwarf: &gimli::Dwarf<R>,
) -> Option<Location> {
    match attr.value() {
        gimli::AttributeValue::Exprloc(ref data) => parse_expression_location(data, unit.encoding()),
        value => {
            let mut locations = dwarf.attr_locations(unit, value).ok()??;
            let entry = locations.next().ok()??;
            parse_expression_location(&entry.data, unit.encoding())
        }
    }
}

fn get_location_list<R: Reader>(
    attr: &gimli::Attribute<R>,
    unit: &gimli::Unit<R>,
    dwarf: &gimli::Dwarf<R>,
) -> Vec<ScopedLocation> {
    let mut result = Vec::new();
    if let Ok(Some(mut locations)) = dwarf.attr_locations(unit, attr.value()) {
        while let Ok(Some(entry)) = locations.next() {
            if let Some(location) = parse_expression_location(&entry.data, unit.encoding()) {
                result.push(ScopedLocation {
                    range_start: entry.range.begin.try_into().unwrap(),
                    range_end: entry.range.end.try_into().unwrap(),
                    location,
                });
            }
        }
...
fn get_attr_value<R: Reader>(
    ...
    match value {
        ...
        gimli::AttributeValue::UnitRef(offset) => Ok(DebugValue::Size(offset.to_unit_section_offset(unit).0)),
        ...
        gimli::AttributeValue::Sdata(_data) => Ok(DebugValue::NoVal),
